...
tic
% INITIALIZEATION
m = mass*ones((nely*nelx)-ceil(nely/2),1);
cut = 1.01*Diam;
xi = zeros(length(m),1);  yi = xi;
Fxe = zeros(length(m),1);   Fye = Fxe;
Fye(ceil(length(m)/2)) = -1;
% Initial geometry
for j = 1:nely
    for i = 1:nelx-mod(j,2)
        xi((j-1)*nelx+i-floor((j)/2)) = Diam*(i-0.5)+(1-mod(j+1,2))*Diam/2-(nelx*Diam)/2;
        yi((j-1)*nelx+i-floor((j)/2)) = Diam*((j-1)*cos(pi/6)+0.0);
    end
end
